...
class SeanceDAO
{
    ...
    public function create($seance) {
    	$query = 'INSERT INTO tseance(pk_timestamp_seance, pkfk_nom_salle, pkfk_id_film, doublage) VALUES(:dateSeance, :nomSalle, :nomFilm, :doublage)';
    	$connection = $this->getDao ()->getConnexion ();
    ...
    public function update($seance){
    	$query = 'UPDATE tseance SET doublage = :doublage, pkfk_id_film = :nomFilm WHERE pk_timestamp_seance = :dateSeance, pkfk_nom_salle = :nomSalle ';
    	$connection = $this->getDao ()->getConnexion ();
    ...
    public static function bind($donnees){
    	...
    	$seance->setFilm($this->getDao()->getFilmDAO()->find($donnees['pkfk_id_film']));
